const APP_NAME = "Cortinify";
const DEFAULT_FADE_SECONDS = 6;
const NEXT_TRACK_DELAY_MS = 150;
const FADE_SECONDS_KEY = "cortinify:fade-seconds";
const PANEL_VISIBLE_KEY = "cortinify:panel-visible";
const PANEL_COLLAPSED_KEY = "cortinify:panel-collapsed";
const CORTINA_TAGS_KEY = "cortinify:cortina-tags";
const PLAYBAR_ICON = "skip-forward";
const PANEL_ID = "cortinify-panel";
...
type PanelRoot = HTMLElement & {
  cortinifyCleanup?: () => void;
};
...
type CortinaTagStore = Record<string, true>;
...
function getSavedFadeSeconds(): number {
...
function saveFadeSeconds(value: number): void {
  Spicetify.LocalStorage?.set(FADE_SECONDS_KEY, String(clamp(value, 0, 60)));
}

function getCortinaTagKey(playlistUri: string, trackUri: string): string {
  return `${playlistUri}::${trackUri}`;
}

function getCortinaTags(): CortinaTagStore {
  try {
    const raw = Spicetify.LocalStorage?.get(CORTINA_TAGS_KEY);
    return raw ? (JSON.parse(raw) as CortinaTagStore) : {};
  } catch {
    return {};
  }
}

function saveCortinaTags(tags: CortinaTagStore): void {
...
function resolvePlaylistUri(contextUri?: string): string | null {
...
function tagTrackAsCortina(playlistUri: string, trackUri: string): void {
  const tags = getCortinaTags();
  tags[getCortinaTagKey(playlistUri, trackUri)] = true;
  saveCortinaTags(tags);
}

function untagTrackAsCortina(playlistUri: string, trackUri: string): void {
  const tags = getCortinaTags();
  delete tags[getCortinaTagKey(playlistUri, trackUri)];
  saveCortinaTags(tags);
}

function isTrackTaggedAsCortina(playlistUri: string, trackUri: string): boolean {
  const tags = getCortinaTags();
  return Boolean(tags[getCortinaTagKey(playlistUri, trackUri)]);
}

function getAlbumArtUrl(): string | null {
  const meta = Spicetify.Player?.data?.item?.metadata;
  if (!meta) {
    return null;
  }

  const withUrl = meta as {
  ...
  return (
    withUrl.image_large_url ??
    withUrl.image_xlarge_url ??
    withUrl.image_url ??
    null
  );
}

function getCurrentTrackMetadata(): { title: string; artist: string } {
  const meta = Spicetify.Player?.data?.item?.metadata;
...
function getCurrentTrackTiming(): {
  elapsed: string;
  total: string;
  remaining: string;
} {
  const elapsedMs = Spicetify.Player?.getProgress?.() ?? 0;
  ...
  return {
    ...
    remaining: `-${formatMilliseconds(remainingMs)}`
  };
}
...
async function fadeOutAndSkip(
  fadeSeconds: number,
  onProgress?: (state: FadeProgress) => void
): Promise<void> {
  if (isFading) {
  ...
  const stepDelayMs = steps === 0 ? 0 : (safeFadeSeconds * 1000) / steps;
...
function createPanelManager(): PanelManager {
  const existingRoot = document.getElementById(PANEL_ID) as PanelRoot | null;
  if (existingRoot) {
...
`;
  ...
  const syncDuration = (): void => {
    ...
    for (let index = 0; index < presetButtons.length; index += 1) {
      ...
      const isActive = Math.abs(state.fadeSeconds - presetValues[index]) < 0.001;
      button.style.background = isActive
        ? "var(--spice-button)"
        : "rgba(var(--spice-rgb-text), 0.08)";
      button.style.color = isActive
        ? "var(--spice-button-text)"
        : "var(--spice-text)";
    }
  };
  ...
  (root as PanelRoot).cortinifyCleanup = () => {
  ...
  new Spicetify.ContextMenu.Item(
    ...
    (uris, _uids, contextUri) => {
      ...
      return (
        ...
        isTrackUri(uris[0]) &&
        Boolean(playlistUri) &&
        isTrackTaggedAsCortina(playlistUri, uris[0])
      );
    },
